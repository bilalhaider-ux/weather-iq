...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0e0e0f] text-[#e5e2e3] font-sans selection:bg-primary selection:text-on-primary">
      {/* Sidebar Navigation */}
      <aside className={`h-screen w-64 fixed left-0 top-0 flex flex-col p-8 border-r border-white/10 bg-[#131314]/30 backdrop-blur-xl shadow-2xl z-50 transition-transform duration-300 md:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        {/* Mobile Close Button */}
        <button
          onClick={() => setSidebarOpen(false)}
          className="absolute top-6 right-6 text-on-surface-variant hover:text-white md:hidden cursor-pointer bg-transparent border-none"
        >
          <span className="material-symbols-outlined">close</span>
        </button>

        <div className="mb-12">
          <h1 className="text-3xl font-black text-primary tracking-tight cursor-pointer" style={{ fontFamily: "Outfit" }} onClick={() => { setActiveTab("Dashboard"); setSidebarOpen(false); }}>
            WeatherIQ
          </h1>
        ...
        <div className="mt-auto space-y-4">
          <button
            onClick={() => { setShowProModal(true); setSidebarOpen(false); }}
          ...
          <div className="pt-4 border-t border-white/10 space-y-2">
            <a
              onClick={(e) => { e.preventDefault(); setShowSupportModal(true); setSidebarOpen(false); }}
      ...
      {/* Top Navbar */}
      <header className="fixed top-0 right-0 left-0 md:left-64 h-16 flex justify-between items-center px-4 md:px-8 bg-[#131314]/10 backdrop-blur-md border-b border-white/10 z-40">
        {mobileSearchActive ? (
          <form onSubmit={handleSearchSubmit} className="flex flex-1 items-center bg-white/5 border border-white/10 focus-within:border-primary/50 focus-within:ring-1 focus-within:ring-primary/20 rounded-full px-4 py-1.5 transition-all">
            <button
              type="button"
              onClick={() => setMobileSearchActive(false)}
              className="material-symbols-outlined text-on-surface-variant mr-2 text-lg hover:text-white cursor-pointer bg-transparent border-none"
            >
              arrow_back
            </button>
            <input
          ...
          <>
            <div className="flex items-center gap-3 md:gap-8">
              ...
              <button
                ...
                className="md:hidden text-on-surface-variant hover:text-white p-1 cursor-pointer bg-transparent border-none flex items-center"
              >
                <span className="material-symbols-outlined text-2xl">menu</span>
              </button>
              
              <span className="text-lg md:text-xl font-black text-primary cursor-pointer whitespace-nowrap" style={{ fontFamily: "Outfit" }} onClick={() => setActiveTab("Dashboard")}>
                WeatherIQ AI
              </span>
            ...
            <div className="flex items-center gap-3 md:gap-6">
              ...
              <div className="text-sm text-on-surface-variant font-medium bg-white/5 border border-white/10 rounded-full px-4 py-2 hidden lg:block">
                {liveClock}
              </div>
              <button
                onClick={handleSync}
                disabled={loading}
                className="bg-primary hover:bg-primary-fixed-dim px-3 md:px-4 py-2 rounded-lg text-on-primary text-xs md:text-sm font-bold opacity-90 hover:opacity-100 hover:scale-[1.02] transition-all cursor-pointer flex items-center gap-1.5 disabled:opacity-50"
              >
                <span className={`material-symbols-outlined text-sm ${loading ? 'animate-spin' : ''}`}>
                  sync
                </span>
                <span className="hidden sm:inline">Run AI Sync</span>
              </button>
            </div>
          </>
        )}
      ...
      {/* Main Content Area */}
      <main className="ml-0 md:ml-64 mt-16 p-4 md:p-8 min-h-[calc(100vh-4rem)]">
